...
import {
  ArrowLeft, Plus, Search, X, BookImage, Film, Check,
  ChevronLeft, ChevronRight, FolderOpen, Image,
} from 'lucide-react'
import { toast } from 'sonner'
import { templatesApi, libraryApi, searchApi, mediaApi, projectsApi } from '../api/client'
import { FilmStrip } from '../components/assemble/FilmStrip'
import { SlideThumbnail } from '../components/common/SlideCard'
import { Spinner } from '../components/common/Spinner'
import { cn } from '../utils/cn'
import type { Slide, SlideOverlay, MediaAsset, MediaFolder, Project } from '../types'
...
function LibraryPanel({ existingIds, onAdd }: { existingIds: Set<number>; onAdd: (slide: Slide) => void }) {
    ...
    <div className="flex flex-col h-full">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    already ? 'border-brand-400 opacity-60 cursor-default' : 'border-gray-200 hover:border-brand-400 cursor-pointer hover:shadow-sm'
                  )}
                >
                  <img
      ...
      {!isSearching && totalPages > 1 && (
        <div className="flex items-center justify-between px-3 py-2 border-t border-gray-100 shrink-0">
          <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 transition-colors">
            <ChevronLeft className="w-3.5 h-3.5" />
          </button>
          <span className="text-[10px] text-gray-400">{page} / {totalPages}</span>
          <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="p-1 rounded hover:bg-gray-100 disabled:opacity-30 transition-colors">
        ...
        {folderOptions.map(({ key, label }) => (
          <button
            ...
            className={cn(
              ...
              selectedFolder === key ? 'bg-brand-100 text-brand-700 font-medium' : 'text-gray-500 hover:bg-gray-100'
            )}
          >
            {label}
          </button>
            ...
            {assets.map((asset) => (
              ...
              >
                {asset.file_type === 'video' ? (
                  <video src={asset.url} muted className="w-full object-cover" style={{ aspectRatio: '16/9' }} />
                ) : (
                  <img src={asset.url} alt={asset.name} className="w-full object-cover" style={{ aspectRatio: '16/9' }} />
                )}
...
export default function TemplateEditor() {
  ...
  useEffect(() => {
    ...
    const onUp = () => {
      ...
      if (currentTemplateId) {
        templatesApi.update(currentTemplateId, { overlays: overlaysRef.current })
  ...
  const handleAddOverlay = useCallback(async (asset: MediaAsset) => {
    ...
    if (!slide) { toast.error('Выберите слайд'); return }
    ...
    if (currentTemplateId) {
      templatesApi.update(currentTemplateId, { overlays: newOverlays })
        .then(() => queryClient.invalidateQueries({ queryKey: ['templates'] }))
        .catch(() => {})
    }
  ...
  const handleAddSlide = (slide: Slide) => {
    ...
    if (currentTemplateId) {
      templatesApi.update(currentTemplateId, { slide_ids: newSlides.map((s) => s.id) })
        .then(() => queryClient.invalidateQueries({ queryKey: ['templates'] }))
        .catch(() => {})
    }
  }
          ...
          >
            {selectedSlide ? (
              <div className="w-full max-w-4xl">
                <div ref={containerRef} className="relative w-full rounded-xl overflow-hidden shadow-xl border border-gray-200">
