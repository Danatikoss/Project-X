                  ...
                  {selectMode && !added && (
                    <div
                      className={cn(
                        ...
                        isSelected ? 'border-brand-600 bg-brand-900/10' : 'border-transparent hover:border-brand-300'
                      )}
                      onClick={() => toggleSelect(slide)}
                    >
                      <div className={cn(
                        'absolute top-1.5 left-1.5 w-4 h-4 rounded border-2 flex items-center justify-center transition-all',
            ...
            <div className="p-4 border-b border-gray-200">
              ...
              {editingTitle ? (
                ...
                <div className="flex items-start gap-2">
                  <h2 className="flex-1 text-sm font-semibold text-gray-900 leading-snug">
              ...
              {!isManual && (
                <p className="text-[11px] text-gray-400 mt-1 line-clamp-2">{assembly?.prompt}</p>
              )}
            </div>
            ...
            {selectedSlide ? (
              <div className="p-4 border-b border-gray-200">
                ...
                {selectedSlide.summary && (
                  <p className="text-xs text-gray-500 mb-2 line-clamp-3">{selectedSlide.summary}</p>
                )}
                {selectedSlide.labels && selectedSlide.labels.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {selectedSlide.labels.map((lbl) => (
                      <span key={lbl} className="text-[10px] px-1.5 py-0.5 bg-teal-50 text-teal-700 rounded-full border border-teal-200">
                        {lbl}
                      </span>
                    ))}
                  </div>
                ...
                {selectedSlide.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    {selectedSlide.tags.map((tag) => (
                      <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-brand-50 text-brand-700 rounded-full">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                {selectedSlide.source_filename && (
                  <p className="text-[10px] text-gray-400 mt-1 truncate">
                    Источник: {selectedSlide.source_filename}
                  </p>
                )}
                <button
            ...
            <div className="p-4 mt-auto">
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className={cn(
                    'w-full flex items-center justify-center gap-2 py-2.5 rounded-xl',
                    'bg-brand-900 text-white text-sm font-medium',
