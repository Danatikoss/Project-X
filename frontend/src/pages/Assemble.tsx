...
import { assemblyApi, libraryApi, mediaApi, projectsApi, searchApi } from "../api/client";
...
function ShareModal({
	...
	const [tab, setTab] = useState<"view" | "edit" | "slideshow">("view");
	...
	const generateEditLink = async () => {
		setLoadingEdit(true);
		try {
			const { edit_token } = await assemblyApi.shareEdit(assemblyId);
			setEditToken(edit_token);
		} catch {
			toast.error("Не удалось создать ссылку для редактирования");
		} finally {
			...
			<div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
				...
				<div className="px-6 py-4 min-h-[130px]">
					...
					{tab === "slideshow" && (
						<div className="space-y-3">
							<p className="text-xs text-gray-500">
								Ссылка открывает слайды сразу в полноэкранном режиме. Удобно для демонстраций.
							</p>
							{loadingView ? (
								<div className="flex items-center gap-2 py-2">
									<Spinner size="sm" />
								...
								<ShareLinkRow url={`${viewUrl}?slideshow=1`} />
							) : (
								<p className="text-xs text-red-400">Не удалось создать ссылку. Попробуйте снова.</p>
							)}
							<div className="flex items-center gap-1.5 pt-1">
								<div className="w-1.5 h-1.5 rounded-full bg-violet-400" />
								<span className="text-xs text-gray-400">Открывается слайд-шоу — без интерфейса</span>
							</div>
						</div>
					)}
				</div>
		...
		<div ref={ref} className="relative">
			...
			{open && (
				<div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 py-1 min-w-[140px]">
					...
					>
						<Download className="w-4 h-4 text-gray-400" /> PPTX
					</button>
					<button
						onClick={() => {
							onExport("pdf");
...
export default function Assemble() {
	...
	const assemblyId = parseInt(id || "0", 10);
	...
	const handleAddMultiple = (slides: Slide[]) => {
		...
		updateMutation.mutate({ slide_ids: newSlides.map((s) => s.id) });
		toast.success(
			`Добавлено ${toAdd.length} ${toAdd.length === 1 ? "слайд" : toAdd.length < 5 ? "слайда" : "слайдов"}`,
			{ duration: 2000 }
		);
	};
	...
	if (!assembly && !isLoading) {
		return (
			<div className="flex flex-col items-center justify-center h-full gap-4 text-gray-400">
				...
				<button onClick={() => navigate("/dashboard")} className="text-brand-700 text-sm underline">
					На главную
				</button>
			</div>
		);
	...
	const editingSlide =
		editingSlideId !== null ? localSlides.find((s) => s.id === editingSlideId) : null;
	const existingIds = new Set(localSlides.map((s) => s.id));
	const _isManual = assembly?.prompt === "(создано вручную)";
			...
			<div className="flex-1 flex overflow-hidden">
				...
				<aside
					className={cn(
						...
						rightCollapsed
							? "w-[48px] flex-col"
							: cn("flex-col", rightTab === "library" ? "w-[340px]" : "w-[300px]")
					)}
				>
