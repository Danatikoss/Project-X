		...
		>
			...
			>
				...
				<div className="w-full h-32 rounded-xl bg-slate-50 border border-slate-200 overflow-hidden flex items-center justify-center mb-4">
					{previewUrl ? (
						<img src={previewUrl} className="h-full w-full object-contain" alt="" />
					) : (
				...
				{uploading && (
					<div className="mt-4">
						<div className="flex items-center justify-between mb-1">
							...
							<span className="text-xs font-semibold text-brand-600">{uploadProgress}%</span>
						</div>
				...
				<div className="flex gap-2 mt-5">
					<button
						onClick={onCancel}
						disabled={uploading}
						className="flex-1 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-40 transition-colors"
					>
						Отмена
					</button>
					<button
...
function UploadQueuePanel({ items, onDismiss }: { items: QueueItem[]; onDismiss: () => void }) {
	if (items.length === 0) return null;
	const allDone = items.every((i) => i.status === "done" || i.status === "error");

	return (
		<div className="fixed bottom-5 right-5 z-40 w-72 bg-white rounded-2xl border border-slate-200 shadow-card-hover overflow-hidden animate-slide-up">
				...
				{items.map((item) => (
					<div key={item.id} className="px-4 py-2.5">
						...
						{(item.status === "uploading" || item.status === "done") && (
							<div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden ml-5">
								<div
									className={cn(
										"h-full rounded-full transition-all duration-200",
										item.status === "done" ? "bg-emerald-400" : "bg-gradient-brand"
									)}
...
export default function Media() {
	...
	const uploadMutation = useMutation({
		mutationFn: ({
			...
			name,
			folderId,
		}: { file: File; name: string; folderId: number | null }) =>
			mediaApi.upload(file, name, folderId ?? undefined, (pct) => setUploadProgress(pct)),
		onSuccess: () => {
	...
	const processQueue = useCallback(
		async (queue: QueueItem[], folderId: number | null) => {
			...
			for (const item of queue) {
				...
				try {
					...
					await mediaApi.upload(optimized, item.name, folderId ?? undefined, (pct) => {
						setUploadQueue((prev) =>
	...
	const handleFileSelect = useCallback(
		(files: FileList | null) => {
			...
			if (validFiles.length === 1) {
				// Single file → name/folder modal
				setPendingFile(validFiles[0]);
			} else {
				// Multiple files → batch queue (auto-names, current folder)
				const folderId = typeof activeFolder === "number" ? activeFolder : null;
				...
				processQueue(items, folderId);
			}
		},
		[activeFolder, processQueue]
	);
		...
		>
			<div className="flex h-full">
				...
				<div className="flex-1 flex flex-col min-w-0">
					...
					<div
						className={cn(
							"flex-1 overflow-auto p-5 relative transition-colors",
							isDragOver && "bg-brand-50/50"
						)}
					...
					>
						{/* Drop overlay */}
						{isDragOver && (
							<div className="absolute inset-3 z-20 rounded-2xl border-2 border-dashed border-brand-400 bg-brand-50/80 flex flex-col items-center justify-center pointer-events-none">
								<Upload className="w-10 h-10 text-brand-400 mb-2" />
								<p className="text-sm font-semibold text-brand-700">Отпустите для загрузки</p>
							</div>
						)}
						{assetsLoading ? (
			...
			{pendingFile && (
				<UploadModal
					...
					onSubmit={async (name, folderId) => {
						const optimized = await optimizeImage(pendingFile);
						uploadMutation.mutate({ file: optimized, name, folderId });
					}}
					onCancel={() => { if (!uploadMutation.isPending) setPendingFile(null); }}
				/>
			)}
