...
function UploadModal({ file, folders, currentFolderId, onSubmit, onCancel, uploading }: UploadModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onCancel}>
      ...
      >
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Название</label>
            <input
              autoFocus
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') onSubmit(name, folderId) }}
          ...
          {folders.length > 0 && (
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1.5 uppercase tracking-wide">Папка</label>
              <select
                value={folderId ?? ''}
        ...
        <div className="flex gap-2 mt-5">
          ...
          >
            {uploading ? <Spinner size="sm" className="border-white border-t-transparent" /> : <Upload className="w-4 h-4" />}
            Загрузить
          </button>
        </div>
      </div>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragging ? 'opacity-40 scale-95 shadow-lg' : 'hover:border-brand-300 hover:shadow-card-hover hover:-translate-y-0.5',
      )}
    >
      {/* Preview area */}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragOver ? 'bg-brand-100 border-2 border-brand-400 border-dashed scale-[1.01]' :
    ...
    >
      {isActive
        ...
        : <Folder className={cn('w-4 h-4 shrink-0', isDragOver ? 'text-brand-600' : 'text-slate-400')} />
      }
    ...
    >
          ...
          <div ref={unfiledRef}>
            <button
              ...
              className={cn(
                ...
                dragOverFolder === null && draggingAsset ? 'bg-brand-100 border-2 border-dashed border-brand-400' : ''
              )}
            >
              <Folder className={cn('w-4 h-4 shrink-0', activeFolder === 'unfiled' ? 'text-brand-600' : 'text-slate-400')} />
              Без папки
            </button>
          </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                {assets.map((asset) => (
                  <MediaCard
                    ...
                    onRename={(name) => updateAssetMutation.mutate({ id: asset.id, data: { name } })}
                  />
                ))}
              </div>
            )}
        ...
        {draggingAsset && (
          <div className="w-28 rounded-xl border-2 border-brand-400 bg-white shadow-xl overflow-hidden opacity-90">
            ...
            <div className="px-2 py-1.5">
              <p className="text-[10px] font-semibold text-slate-700 truncate">{draggingAsset.name}</p>
            </div>
          </div>
        )}
      </DragOverlay>
      ...
      {pendingFile && (
        <UploadModal
          ...
          onSubmit={(name, folderId) => uploadMutation.mutate({ file: pendingFile, name, folderId })}
