...
import {
  ...
  FileText, Wand2, ShieldCheck, ChevronLeft, ChevronRight, Sun, Moon,
} from 'lucide-react'
    ...
    <div className="flex h-screen bg-surface overflow-hidden">
      ...
      <aside
        className={cn(
          ...
          isDark
            ? 'bg-sidebar border-r border-white/5 shadow-sidebar'
      ...
      >
        ...
        <div className={cn(
          ...
          {!collapsed && (
            <>
              <span className={cn('font-bold text-base tracking-tight', isDark ? 'text-white' : 'text-gray-900')}>
                SLIDEX
              </span>
        ...
        {/* User + collapse toggle + theme toggle */}
        <div className={cn(
          'px-2 py-3 flex flex-col gap-1',
          isDark ? 'border-t border-white/5' : 'border-t border-gray-200'
        )}>
          {!collapsed && (
            <NavLink
              to="/profile"
              className="flex items-center gap-2.5 px-2 py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors group"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {user?.email && user?.name && (
                  <p className={cn('text-[10px] truncate', isDark ? 'text-slate-500' : 'text-gray-400')}>
                    {user.email}
                  </p>
                )}
              </div>
          ...
          {collapsed && (
            <NavLink
              to="/profile"
              title="Профиль"
              className="flex items-center justify-center py-2 rounded-xl hover:bg-gray-100 dark:hover:bg-white/5 transition-colors"
            >
      ...
      {/* Bottom nav — mobile */}
      <nav className={cn(
        'md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center px-1',
        isDark ? 'bg-sidebar border-t border-white/10' : 'bg-white border-t border-gray-200 shadow-[0_-2px_8px_rgba(0,0,0,0.06)]'
      )}>
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            key={to}
